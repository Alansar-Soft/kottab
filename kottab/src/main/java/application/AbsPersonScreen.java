...
public abstract class AbsPersonScreen<T extends Person> implements IFileScreen<T>
{
    ...
    @Override
    public AnsarTable<T> createListViewTable()
    {
        ...
        countryCol.config("country", Translator::translate);
        cityCol = new AnsarTableColumn<>("city");
        cityCol.config("city", Translator::translate);
        townCol = new AnsarTableColumn<>("town");
        townCol.config("town", Translator::translate);
        azharStudentCol = new AnsarTableColumn<>("azharStudent");
        azharStudentCol.config("azharStudentVal");
        table.getColumns().addAll(codeCol, registrationDateCol, nameCol, birthdateCol, firstPhoneNoCol, secondPhoneNoCol, countryCol, cityCol, townCol, azharStudentCol);
    ...
    public T createEntity()
    {
        ...
        Long id = table.getItems().stream().filter(i -> ObjectChecker.areEqual(i.getCode(), code)).map(i -> i.getId()).findFirst().orElse(null);
        person.setId(id);
        person.setCode(code);
        ...
        person.setAzharStudent(studyInAzharBox.fetchValue());
        return (T) person;
    }
